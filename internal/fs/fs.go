	...
	if ic.Object == nil {
		...
		for {
			...
			if !ok {
				...
				fmt.Printf("ImplicitInode firsttime logic is invoked for %v and %v\n", in.ID(), ic.FullName)
				in.Lock()
				return
			}

			fmt.Printf("ImplicitInode logic is invoked for %v and %v\n", in.ID(), ic.FullName)
			fmt.Printf("ImplicitInode logic, fs.mu.unlock %v and %v\n", in.ID(), ic.FullName)
			fs.mu.Unlock()
			fmt.Printf("ImplicitInode logic, existingInode lock %v and %v\n", in.ID(), ic.FullName)
			in.Lock()
			fmt.Printf("ImplicitInode logic, fs.mu.lock called %v and %v\n", in.ID(), ic.FullName)
			fs.mu.Lock()
			fmt.Printf("ImplicitInode logic, fs.mu.lock acquired %v and %v \n", in.ID(), ic.FullName)

			if fs.implicitDirInodes[ic.FullName] != in {
				fmt.Printf("ImplicitInode logic, inode unlock, continue %v and %v\n", in.ID(), ic.FullName)
				in.Unlock()
				continue
			}
