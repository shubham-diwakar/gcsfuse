...
type fileSystem struct {
	...
	localFileInodes map[inode.Name]*inode.FileInode
...
func (fs *fileSystem) lookUpOrCreateLocalFileInode(ic inode.Core) (in inode.Inode) {
	defer func() {
		if in != nil {
			in.IncrementLookupCount()
		}
	...
	in, ok := fs.localFileInodes[ic.FullName]

	if !ok {
		in = fs.mintInode(ic)
		in.Lock()
		fileInode := in.(*inode.FileInode)
		fileInode.SetLocalFile(true)
		fs.localFileInodes[in.Name()] = fileInode
		return fileInode
	}

	// TODO: why do we need locks.
	in.Lock()
	return in
}
...
func (fs *fileSystem) createLocalFile(
	...
	parent.Lock()
	// TODO: replace this with just stat call. lookupchild makes a stat and list call.
	...
	//TODO: CreateFile needs to be added directory inode to support ReadDir calls.
	child = fs.lookUpOrCreateLocalFileInode(*result)
...
func (fs *fileSystem) CreateFile(
	...
	// Create the child.
	if fs.enableLocalFile {
		child, err = fs.createLocalFile(ctx, op.Parent, op.Name, op.Mode)
	} else {
		child, err = fs.createFile(ctx, op.Parent, op.Name, op.Mode)
	}
