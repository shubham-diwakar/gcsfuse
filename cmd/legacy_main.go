...
func createStorageHandle(flags *flagStorage, mountConfig *config.MountConfig,
	commonConfig *cfg.Config, userAgent string) (storageHandle storage.StorageHandle, err error) {
	storageClientConfig := storageutil.StorageClientConfig{
		...
		KeyFile:                    string(commonConfig.GcsAuth.KeyFile),
...
func mountWithArgs(
	...
	commonConfig *cfg.Config) (mfs *fuse.MountedFileSystem, err error) {
	...
	if bucketName != canned.FakeBucketName {
		...
		storageHandle, err = createStorageHandle(flags, mountConfig, commonConfig, userAgent)
		if err != nil {
...
func runCLIApp(c *cli.Context) (err error) {
	...
	resolvedConfig, err := PopulateConfigFromLegacyFlags(flags, mountConfig)
	if err != nil {
		return fmt.Errorf("PopulateConfigFromLegacyFlags failed: %v", err)
	}

	config.OverrideWithLoggingFlags(mountConfig, flags.DebugFuse, flags.DebugGCS, flags.DebugMutex)
	...
	{
		mfs, err = mountWithArgs(bucketName, mountPoint, flags, mountConfig, resolvedConfig)
