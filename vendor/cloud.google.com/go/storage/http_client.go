...
func (c *httpStorageClient) NewRangeReader(ctx context.Context, params *newRangeReaderParams, opts ...storageOption) (r *Reader, err error) {
	bucketSegment := httputil.EncodePathSegment(params.bucket)
	objectSegment := httputil.EncodePathSegment(params.object)
	opaque := fmt.Sprintf(
	...
	byteRange := &gcs.ByteRange{
		Start: uint64(params.offset),
		Limit: uint64(params.offset + params.length),
	}
	...
	if byteRange != nil {
		var v string
		v, _ = makeRangeHeaderValue(*byteRange)
		httpReq.Header.Set("Range", v)
	}

	// Call the server.
	httpRes, err := params.httpClient.Do(httpReq)
	if err != nil {
		return
	}
