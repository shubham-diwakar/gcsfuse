...
func FindDefaultCredentialsWithParams(ctx context.Context, params CredentialsParams) (creds *Credentials, err error) {
	...
	if filename := os.Getenv(envVar); filename != "" {
		...
		err = fmt.Errorf("reading the creds file: %v", err)
		if err != nil {
	...
	if appengineTokenFunc != nil {
		err = fmt.Errorf("reading the creds file from appEngine: %v", err)
		return &Credentials{
	...
	if metadata.OnGCE() {
		err = fmt.Errorf("reading the creds file from metadata.onGCE: %v", err)
