...
func list1MFiles(t *testing.T, mode string) {
	...
	err = static_mounting.MountGcsfuseForBucket("gcsfuse-metadata-prefetch-load-test", flags)
	if err != nil {
...
func oneMillionDirOneFileEach(t *testing.T, mode string) {
	...
	flags := []string{
		"--only-dir=1M_dirs_one_file_each",
		"--implicit-dirs",
		"--metadata-prefetch-on-mount=" + mode,
		"--config-file=" + configForInfiniteMetadataCache(t, "config.yaml"),
	}
	...
	assert := assert.New(t)
	if assert.Nil(err) {
		assert.Equal(1_000_000, len(objs))
		for _, d := range objs {
			assert.True(d.IsDir())
...
func dirs1000With1000FilesEach(t *testing.T, mode string) {
	...
	flags := []string{
		"--only-dir=1000dirs_1000files_each",
		"--metadata-prefetch-on-mount=" + mode,
		"--implicit-dirs",
		"--config-file=" + configForInfiniteMetadataCache(t, "config.yaml"),
	}
...
func TestMetadataPrefetch(t *testing.T) {
	...
	for _, k := range tests {
		for _, m := range []string{"disabled", "sync", "async"} {
			testName := k.name + "_" + m
			t.Run(testName, func(t *testing.T) {
				startTime := time.Now()
				k.testFn(t, m)
				diff := time.Now().Sub(startTime)
				log.Printf("Time taken by %s test: %s\n", testName, diff)
			},
			)
		}
	}
